...
if __name__ == "__main__":
    ...
    for name, data in [("laptops_train",
        ...
        os.makedirs(os.path.join("../processed", "semeval", "restaurant"), exist_ok=True)
        os.makedirs(os.path.join("../processed", "semeval", "laptop"), exist_ok=True)
        ...
        if "train" in name:
            ...
            train = p[dev_idx:]
            if "laptop" in name:
                with open(os.path.join("../processed", "semeval", "laptop", "train.json"), "w") as out:
                    json.dump(train, out)
                with open(os.path.join("../processed", "semeval", "laptop", "dev.json"), "w") as out:
                    json.dump(dev, out)
            if "restaurant" in name:
                with open(os.path.join("../processed", "semeval", "restaurant", "train.json"), "w") as out:
                    json.dump(train, out)
                with open(os.path.join("../processed", "semeval", "restaurant", "dev.json"), "w") as out:
                    json.dump(dev, out)
        else:
            if "laptop" in name:
                with open(os.path.join("../processed", "semeval", "laptop", "test.json"), "w") as out:
                    json.dump(p, out)
            if "restaurant" in name:
                with open(os.path.join("../processed", "semeval", "restaurant", "test.json"), "w") as out:
                    json.dump(p, out)
