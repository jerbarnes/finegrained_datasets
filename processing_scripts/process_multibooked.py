...
if __name__ == "__main__":
    ...
    parser.add_argument("--datasplit", default="70/10/20", help="train/dev/test split. Must add up to 100. Ex.: 70/10/20")
    ...
    for file in os.listdir(args.indir):
        ...
        #print(file)
        #print(len(opinions))
        dataset.extend(opinions)

    sp = [int(i) * .01 for i in args.datasplit.split("/")]
    train_split = sp[0]
    dev_split = sp[0] + sp[1]
    #train_split, dev_split = 0.7, 0.8
    train_idx = int(len(dataset) * train_split)
    dev_idx = int(len(dataset) * dev_split)

    train = dataset[:train_idx]
    dev = dataset[train_idx:dev_idx]
    test = dataset[dev_idx:]
