...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.join("../processed", "wang"), exist_ok=True)
    ...
    for name, corpus in [("train", train_idxs), ("test", test_idxs)]:
        ...
        if name is "train":
            ...
            with open(os.path.join("../processed", "wang", "train.json"), "w") as out:
                json.dump(train, out)
            with open(os.path.join("../processed", "wang", "dev.json"), "w") as out:
                json.dump(dev, out)
        else:
            with open(os.path.join("../processed", "wang", "{0}.json".format(name)), "w") as out:
